...
RSpec.describe PaymentIngestionsHelper, type: :helper do
  describe "#payment_ingestion_alias_proposal" do
    ...
    it "returns nil if tenant/party is nil" do
      ingestion = build(:payment_ingestion, user: user, party: nil)
      expect(helper.payment_ingestion_alias_proposal(ingestion)).to be_nil
    end

    it "returns payer_name if it is an alias candidate" do
      ingestion = build(:payment_ingestion, user: user, party: party, payer_name: "Jane S. Smith", payer_username: nil)
    ...
    it "returns payer_username if payer_name is not candidate but payer_username is candidate" do
      ingestion = build(:payment_ingestion, user: user, party: party, payer_name: "Jane Smith", payer_username: "@janesmith")
    ...
    it "returns nil if neither payer_name nor payer_username are candidates" do
      ingestion = build(:payment_ingestion, user: user, party: party, payer_name: "Jane Smith", payer_username: "")
      expect(helper.payment_ingestion_alias_proposal(ingestion)).to be_nil
    end
  end
end
